...
def get_avg_salary(vacancy: Vacancy) -> Union[int, float]:
...
def initialization_menu() -> UpdateJSON:
...
def search_menu(api_hh: HH) -> List:
    ...
    while True:
        ...
        if choice == "1":
            result = search_vacancies(api_hh, search_in_city=False)
            return result
        elif choice == "2":
            result = search_vacancies(api_hh, search_in_city=True)
            return result


def search_vacancies(api_hh: HH, search_in_city: bool) -> List:
    ...
    if search_in_city:
        vacancies = api_hh.get_vacancies(query, city_name)
    else:
        vacancies = api_hh.get_vacancies(query)
